class Vehicle {
  ...
  separate(boids) {
    ...
    let count = 0;
    // Examine other boids to see if they're too close
    for (let i = 0; i < boids.length; i++) {
      ...
      if (d > 0 && d < desiredSeparation) {
        ...
        diff.normalize();
        diff.div(d); // weight by distance: closer neighbours contribute stronger
        steer.add(diff);
        count++;
      }
    }
    ...
    // If the repulsion force is greater than 0
    if (steer.mag() > 0) {
